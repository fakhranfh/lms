<div x-data="{
        hasSelectionPersistence: {{ $hasSelectionPersistence ? 'true' : 'false' }},
        selectionShowUrl: {{ $selectionShowUrl ? "'{$selectionShowUrl}'" : 'null' }},
        selectionUpdateUrl: {{ $selectionUpdateUrl ? "'{$selectionUpdateUrl}'" : 'null' }},
        selectionUpdateManyUrl: {{ $selectionUpdateManyUrl ? "'{$selectionUpdateManyUrl}'" : 'null' }},
        selectionClearUrl: {{ $selectionClearUrl ? "'{$selectionClearUrl}'" : 'null' }},
        ...
        init() {
            if (! this.hasSelectionPersistence) {
                return;
            }

            fetch(this.selectionShowUrl, { headers: { 'Accept': 'application/json' } })
        ...
        toggleSelectAll(checked) {
            ...
            fetch(this.selectionUpdateManyUrl, {
                method: 'POST',
        ...
        persistSelection(ids) {
            ...
            fetch(this.selectionClearUrl, {
                ...
                return fetch(this.selectionUpdateManyUrl, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': this.csrfToken(), 'Accept': 'application/json' },
...
>
    <x-ui.livewire-data-table
        ...
        :selectable="auth()->user()->can($permissionPrefix.'.delete')"
        :showPagination="false"
    >
        @can("{$permissionPrefix}.delete")
            <x-slot:selectAll>
                <input type="checkbox" :checked="allOnPageSelected" @change="toggleSelectAll($event.target.checked)" aria-label="Select all {{ $permissionPrefix }} on this page">
            </x-slot:selectAll>
                    ...
                    <div class="flex items-center gap-space-md flex-wrap">
                        ...
                        <template x-if="!selectAllMatching && allOnPageSelected && matchingCount > selected.length">
                            <button type="button" @click="selectAllMatching = true" class="font-label-sm text-label-sm text-primary hover:underline">
                                <span x-text="`Select all ${matchingCount} {{ $permissionPrefix }}`"></span>
                            </button>
                        </template>

                        <template x-if="selectAllMatching">
                            <p class="font-label-sm text-label-sm text-on-surface-variant">
                                <span x-text="`All ${matchingCount} {{ $permissionPrefix }} selected.`"></span>
        ...
        @if ($loaded)
            @forelse ($items as $user)
                <x-users.partials.table-row :user="$user" :entity="$entity" :permission-prefix="$permissionPrefix" />
            @empty
                <tr>
                    <td colspan="4" class="px-space-lg py-space-lg text-center text-on-surface-variant">
                        No {{ $permissionPrefix }} found.
                    </td>
                </tr>
            @endforelse
        @endif
    ...
    <x-ui.modal show="showDeleteModal" onClose="showDeleteModal = false" maxWidth="max-w-sm">
        <div class="bg-surface border border-outline-variant rounded-lg shadow-lg p-space-lg space-y-space-lg">
            ...
            <div class="text-center space-y-space-sm">
                <h3 class="font-headline-sm text-headline-sm text-on-surface" x-text="deleteId === null ? 'Delete Selected {{ ucfirst($permissionPrefix) }}' : 'Delete {{ ucfirst($entity) }}'"></h3>
                <p class="font-body-sm text-body-sm text-on-surface-variant" x-text="deleteId === null ? (selectAllMatching ? `Are you sure you want to delete all ${matchingCount} {{ $permissionPrefix }} matching your search? This action cannot be undone.` : `Are you sure you want to delete ${selected.length} selected {{ $entity }}(s)? This action cannot be undone.`) : 'Are you sure you want to delete this {{ $entity }}? This action cannot be undone.'"></p>
            </div>
