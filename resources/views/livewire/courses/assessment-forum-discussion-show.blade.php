...
<div class="space-y-space-lg">
    ...
    <div>
        ...
        <p class="text-body-sm text-on-surface-variant mt-1">Forum Discussion &middot; Weight {{ rtrim(rtrim(number_format($assessment->weight, 2), '0'), '.') }}%</p>
    </div>
                                ...
                                <td class="px-space-lg py-space-md">
                                    ...
                                    @else
                                        <span class="inline-flex items-center gap-1 text-body-sm text-on-surface-variant">
                                            {{ $row['required'] }} posts required
                                            <span class="material-symbols-outlined text-[18px] text-white bg-outline-variant rounded-sm">close</span>
                                        </span>
                                    @endif
                                </td>
                                <td class="px-space-lg py-space-md font-label-md text-label-md text-on-surface">{{ $row['met'] ? '100 pts' : '0 pts' }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="5" class="px-space-lg py-space-lg text-center text-body-sm text-on-surface-variant">No online sessions yet.</td>
                            </tr>
                        @endforelse
                    @else
                        @forelse ($sessionRows as $row)
                            <tr wire:key="session-{{ $row['session']->id }}">
                                ...
                                <td class="px-space-lg py-space-md text-body-sm text-on-surface">{{ $row['session']->date_start_display?->format('d M Y, H:i') }}</td>
                                <td class="px-space-lg py-space-md text-body-sm text-on-surface">{{ $row['session']->date_end_display?->format('d M Y, H:i') }}</td>
                                <td class="px-space-lg py-space-md text-body-sm text-on-surface" colspan="2">{{ $row['metCount'] }} of {{ $row['totalStudents'] }} students met the {{ $row['required'] }}-post requirement</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="5" class="px-space-lg py-space-lg text-center text-body-sm text-on-surface-variant">No online sessions yet.</td>
    ...
    @if ($isStudent)
        <p class="text-body-xs text-on-surface-variant">Score is computed automatically from your forum participation &mdash; no submission is required.</p>
    @endif
</div>
