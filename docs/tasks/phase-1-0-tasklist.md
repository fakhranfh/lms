...
- [x] Confirm migrations use `$table->uuid('id')->primary()` (not `id()`/auto-increment) for every school-scoped table introduced in this phase.
...
- [x] Generate model: `php artisan make:model School --no-interaction` (skip `-m`, migration already exists).
...
- [x] Write a feature/unit test covering: school creation, unique `domain` constraint violation.

## 3. `users` Table — School Scoping
...
- [x] Update `User` model:
  - [x] Apply `HasUuid` trait.
  - [x] Add `belongsTo(School::class)` relationship.
  - [x] Add `tenant_id` to `$fillable` (or set automatically — see Global Scope section).
- [x] Update `UserFactory` to associate a `School` (via factory relationship) on creation.
- [x] Confirm existing Fortify auth flows (login/register) still function against the UUID-keyed, school-scoped `users` table — run the existing auth test suite.

## 4. Global School Scope

- [x] Create `app/Models/Scopes/TenantScope.php` implementing `Illuminate\Database\Eloquent\Scope`:
  - [x] Applies a `where('tenant_id', ...)` constraint using the currently authenticated user's `tenant_id` (or resolved school context — see Resolved Decisions: Subdomain-based School Resolution).
- [x] Create a `BelongsToTenant` trait that:
  - [x] Registers `TenantScope` in the model's `booted()` method.
  - [x] Auto-fills `tenant_id` on `creating` from the current school context (`CurrentTenant` singleton).
- [x] Apply `BelongsToTenant` to every school-scoped model introduced in this phase (`User`; later phases will apply it to `Course`, `Role`, etc.).
- [x] Refactor `CreateNewUser` action to use `CurrentTenant` singleton instead of auto-creating a school. User will auto-join the school resolved from their request subdomain via `ResolveTenantFromDomain` middleware.
- [x] Write tests proving:
  - [x] A query for school-scoped models run as User A never returns rows belonging to School B.
  - [x] Creating a record without explicitly setting `tenant_id` still assigns the correct school automatically.
  - [x] Super Admin / unauthenticated console context (e.g., queue workers, `php artisan tinker`) can still bypass the scope deliberately via `Model::withoutGlobalScope(TenantScope::class)` when required (e.g., cross-school admin operations) — document the escape hatch.
...
- [x] Create `app/Support/TenantDomainResolver.php` utility class:
  ...
  - [x] Return `School` object or null.
...
- [x] Create `TenantController@store` action (on lms.local routes):
  ...
  - [x] Create School record in database with provided domain.
...
- [x] Refactor `CreateNewUser` to use `CurrentTenant::getTenantId()` instead of creating school (remove `School::create()` call).
...
### Subdomain-based School Resolution
**Decision:** Extract school from request subdomain/domain via middleware, store in singleton, apply to all contexts.

**Implementation Status:**
- **Commit 7c84f65** (`feat(school): add global school scope and domain-based resolution`): Implemented `CurrentTenant`, `ResolveTenantFromDomain` middleware, `TenantScope`, `BelongsToTenant` trait, and three-tier domain routing logic (root/admin/school subdomains).
- **Commit 6f0232a** (`refactor(school): move register URL building into service`): Refactored school registration flow.
...
1. **lms.local** (root/landing) — Public area (no school). School registration form, landing page.
...
- [ ] Create `ResolveTenantFromDomain` middleware with three-tier logic:
  ...
  - [ ] **Case 2: lms.local (root)** → `CurrentTenant::setTenantId(null)`. Public area (no school required).
  - [ ] **Case 3: schoolN.lms.local** → Query `tenants` by domain. If found: `CurrentTenant::setTenantId(school.id)`. If not found: throw 404.
...
- [ ] Refactor `CreateNewUser` to use `CurrentTenant::getTenantId()` — user auto-joins the school resolved from the subdomain they're registering on (remove the `School::create()` call added in Section 3).
- [ ] Queue jobs must serialize `tenant_id` explicitly and set it on `CurrentTenant` at start of `handle()`, before touching any school-scoped model.
- [ ] Console commands and Super Admin cross-school actions bypass scoping via `Model::withoutGlobalScope(TenantScope::class)` — never by leaving `CurrentTenant` unset.
...
- [ ] Seed a "Default School" with domain `admin.lms.local` (for admin panel routing; no users assigned).
- [ ] Create `StoreTenantRequest` form request with domain validation (unique, valid hostname format).
- [ ] Create `TenantController` with:
  - [ ] `@store` (POST /register-school) — Create new school school from form.
...
- [ ] Seed admin user in initial migration:
  ...
  - [ ] Role: 'admin' (Spatie).
  - [ ] tenant_id: NULL (invariant: admin must have null school).
...
- [ ] Seed tenants in DatabaseSeeder:
  - `admin.lms.local` → Default School (admin panel access; no regular users).
  - `test-school.lms.local` → Test School (for feature tests).
- [ ] Seed admin user:
