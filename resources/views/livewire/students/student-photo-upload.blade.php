...
<div class="space-y-space-lg" wire:init="loadUsers" x-data="{
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="font-body-sm text-body-sm text-secondary mt-space-xs">Click a student's photo to choose a replacement. Nothing is final until you press Save.</p>
        </div>
    ...
    <div wire:loading.remove wire:target="search,perPage" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-space-md">
        @forelse ($students as $student)
            @php
                $initial = strtoupper(substr($student->name ?: 'U', 0, 1));
                ...
                $currentSrc = $stagedPhotoUrls[$student->id] ?? $student->profile_photo_path ?? $defaultAvatar;
            @endphp
            <div wire:key="photo-{{ $student->id }}" x-data="{ preview: null, uploading: false, progress: 0 }"
                ...
                <label class="relative group/avatar cursor-pointer w-20 h-20 rounded-full overflow-hidden border-2 border-surface-container-low shadow-sm">
                    <img :src="preview || @js($currentSrc)" alt="{{ $student->name }}" class="w-full h-full object-cover" :class="{ 'opacity-50': uploading }">
                    ...
                    @if (isset($stagedPhotoUrls[$student->id]))
                    ...
                    <input type="file" class="hidden" accept="image/jpeg,image/png,image/gif"
                        @change="
                            ...
                            $wire.upload('uploads.{{ $student->id }}', file,
                                () => { uploading = false; },
                                () => { uploading = false; },
                ...
                <div class="text-center">
                    <p class="font-label-sm text-label-sm text-on-surface truncate max-w-[7rem]">{{ $student->name }}</p>
                    <p class="font-body-sm text-body-sm text-secondary truncate max-w-[7rem]">{{ $student->email }}</p>
                </div>
            </div>
        @empty
            <p class="col-span-full text-center font-body-md text-body-md text-secondary py-space-lg">No students found.</p>
        @endforelse
    </div>
    @endif
