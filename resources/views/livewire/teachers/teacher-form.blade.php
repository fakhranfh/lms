...
<div class="w-full" x-data="{
    ...
    x-init="
        ...
        $wire.$on('teacher-autofilled', ({ name: newName, email: newEmail, password: pwd }) => {
            name = newName; email = newEmail; password = pwd; passwordConfirmation = pwd;
        });
    ">

    <div class="mb-space-lg">
        <a href="{{ route('teachers.index') }}" class="text-body-sm text-primary hover:underline inline-flex items-center gap-space-xs">
            <span class="material-symbols-outlined text-[16px]">arrow_back</span>
            Back to Teachers
        </a>
        <h1 class="font-headline-sm text-headline-sm text-on-surface mt-space-sm">
            {{ $this->isEditing() ? 'Edit Teacher - '.$this->user->name : 'New Teacher' }}
        </h1>
    </div>
    ...
    @if ($loginUrl && ! $this->isEditing())
        <div class="mb-space-lg px-gutter py-space-md bg-success/10 border border-success/20 rounded-lg space-y-space-sm" x-data="{ copied: false }">
            <p class="font-label-md text-label-md text-success">Teacher created. Share this one-time login link:</p>
        ...
        <div wire:key="login-url-modal-{{ md5($loginUrl) }}" x-data="{ copied: false, show: true }">
            <x-ui.modal show="show" onClose="show = false" maxWidth="max-w-lg">
                <div class="bg-surface border border-outline-variant rounded-lg shadow-lg p-space-lg space-y-space-md">
                    ...
                    <p class="font-body-sm text-body-sm text-on-surface-variant">Share this one-time login link with the teacher:</p>
                    <div class="flex items-center gap-space-sm">
        ...
        <div>
            ...
            @if ($this->isEditing())
                ...
                <p class="mt-space-xs font-body-sm text-body-sm text-secondary">
                    Generate a new one-time link if the teacher needs another way to log in and set their password.
                </p>
            @else
                <p class="mt-space-xs font-body-sm text-body-sm text-secondary">
                    The teacher will use this one-time link to log in and set their own password.
                </p>
            @endif
            ...
            <div class="bg-surface border border-outline-variant rounded-lg shadow-lg max-w-sm w-full">
                <div class="p-space-lg space-y-space-lg">
                    ...
                    <div class="text-center space-y-space-sm">
                        <h3 class="font-headline-sm text-headline-sm text-on-surface">Unable to Save Teacher</h3>
