...
- **Fixed attendance rule (no configurable requirements)**: attendance only applies to sessions whose `delivery_mode` is `virtual_class` or `offline` — `online` sessions are excluded entirely (not counted in `total_session`/`total_attendance`, not shown in the attendance table, not in scoring scope). `virtual_class`: a student is "attended" if EITHER a `VideoConferenceParticipation` row exists for them on any of that session's video conferences (mere presence of a join record, no minimum-duration check), OR a Teacher manually marked them present. `offline`: attended ONLY via a Teacher's manual "present" mark — there is no auto-derivation for offline sessions. The configurable `AttendanceRequirement` system (model, migration/table, repository, service, `AttendanceRequirementType` enum, the `AttendanceRequirementSettings` screen/route/tests) was removed entirely — no deprecation shim. `course_attendance_settings.minimal_attendance` remains (unrelated setting used by the summary card only).
- **Attendance Management / Attendance (student view)** — one shared component (`AttendanceIndex`, Forum-style role branching, not template-split): Teacher gets a session tab bar (applicable sessions only) + per-session student roster showing delivery mode, the derived/attended boolean, the attendance source (video-conference join vs manual mark, for `virtual_class`), and a manual override form (status/notes) that writes an `attendances` row with `recorded_by` set — this is still how offline attendance and virtual_class overrides get recorded. Student gets a read-only Attendance Summary card (`total_session` / `total_attendance` / `minimal_attendance`, online sessions excluded) plus a per-session table (delivery mode, dates, attend boolean, source). Route `attendance.index` only; the separate `attendance.settings` route/screen was removed. `attendance.manage` (Teacher/School Admin) still gates the manual-override controls on this same screen.
- **`App\Services\AttendanceDerivationService`** rewritten around the fixed rule: `isAttendanceApplicable()`, `isSessionAttended()`, `attendanceSourceForSession()` (`'video_conference'` | `'manual'` | `null`), `applicableSessionsForCourse()`, and `summaryForStudent()` (excludes `online` sessions from totals). The old `isRequirementFulfilled()`/`checklistForSession()`/per-requirement matching methods are gone.
- **`App\Services\AttendanceScoringService::sessionsInScope()`** now also filters out `online` sessions before applying the assessment's date-range scoping (unchanged: sessions whose `date_start` falls in `[start_date, end_date]`, falling back to all applicable course sessions when none match or when dates are null).
