...
class TeacherForm extends Component
{
    ...
    public function mount(UserService $userService, ?string $id = null): void
    {
        if ($id) {
            ...
            abort_if($this->user === null || ! $this->user->hasRole(RoleName::Teacher), 404);
    ...
    public function savedPhotoSrc(): string
    {
        ...
        $initial = strtoupper(substr($this->name ?: 'T', 0, 1));
    ...
    public function autofill(): void
    {
        ...
        $this->dispatch('teacher-autofilled', name: $this->name, email: $this->email, password: $password);
    }
    ...
    private function teacherRoleId(string $schoolId): ?int
    {
    ...
    {
        abort_unless(auth()->user()->can($this->isEditing() ? 'teachers.edit' : 'teachers.create'), 403);
        ...
        $roleId = $schoolId ? $this->teacherRoleId($schoolId) : null;
        ...
        if ($this->isEditing()) {
            ...
            session()->flash('success', __('Teacher updated successfully.'));
        ...
        try {
            if ($trashedUser) {
                $teacher = $userService->restoreUser($trashedUser, $validated, $this->photo, $roleIds);
            } else {
        ...
        $userService->updateProfile($teacher, ['must_change_password' => true]);
        $link = $userLoginLinkService->createLink($teacher, $validated['loginLinkTtlDays'] * 1440);
        $this->loginUrl = $userLoginLinkService->buildLoginUrl($link);
