...
namespace App\Services\AiGrading;
...
/**
 * Shared logic for AI grading providers exposing an OpenAI-compatible
 * chat completions endpoint (Gemini, DeepSeek, and similar).
 */
abstract class AbstractOpenAiCompatibleProvider implements AiGradingProvider
{
    ...
    public function gradeEssay(string $essay, array $rubric, string $prompt): array
    {
        ...
        try {
            ...
            Log::error("{$this->providerName} API connection failed", ['message' => $e->getMessage()]);

            return [
                'success' => false,
                'error' => "{$this->providerName} API timeout or connection error: ".$e->getMessage(),
            ];
        } catch (RuntimeException $e) {
            Log::error("{$this->providerName} API response could not be parsed", ['message' => $e->getMessage()]);
    ...
    public function parseGradingResponse(string $responseText): array
    {
        ...
        if (! is_array($decoded) || ! isset($decoded['score']) || ! isset($decoded['feedback']) || ! is_array($decoded['feedback'])) {
            throw new RuntimeException("{$this->providerName} response did not match the expected grading schema.");
        }
    ...
    private function extractJson(string $responseText): string
    {
        ...
        throw new RuntimeException("No JSON object found in {$this->providerName} response.");
    }
