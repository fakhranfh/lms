...
<div class="w-full space-y-space-lg" wire:init="loadUsers" x-data="{ showGenerateModal: false }" @teachers-generated.window="showGenerateModal = false">
    @if ($successMessage)
    ...
    <div class="flex items-center justify-between">
        <h1 class="font-headline-sm text-headline-sm text-on-surface">Teachers</h1>
        <div class="flex items-center gap-space-md">
            ...
            @endcan
            @can('teachers.create')
                <a href="{{ route('teachers.create') }}" class="px-space-lg py-space-sm bg-primary text-on-primary rounded-lg font-label-md text-label-md hover:opacity-90 transition-opacity">Create Teacher</a>
            @endcan
        </div>
    </div>
    ...
    <x-teachers.table
        :teachers="$teachers"
        :teachers-loaded="$teachersLoaded"
        :sort="$sort"
        :direction="$direction"
    />
        ...
        @can('teachers.create')
            <x-ui.modal show="showGenerateModal" onClose="showGenerateModal = false" maxWidth="max-w-2xl">
                <div class="bg-surface border border-outline-variant rounded-lg shadow-lg p-space-lg">
                    <livewire:teachers.teacher-generate :embedded="true" wire:key="teacher-generate-modal" />
                </div>
            </x-ui.modal>
        @endcan
    @endif
